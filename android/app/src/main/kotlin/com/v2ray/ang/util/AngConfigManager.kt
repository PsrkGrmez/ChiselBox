...
//            }
    ...
    fun importBatchConfig2(servers: String?, subid: String, append: Boolean): String? {
        try {
            if (servers == null) {
                return null
            }
            ...
            servers.lines()
                .reversed()
                .forEach {
                    json = importConfig2(it, subid, removedSelectedServer)
                }
            return json
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    private fun importConfig2(
        ...
        try {
            ...
            if (str.startsWith(EConfigType.VMESS.protocolScheme)) {
                config = ServerConfig.create(EConfigType.VMESS)
                val streamSetting = config.outboundBean?.streamSettings ?: return ""
                ...
                val legacyPattern = "^(.*):(.*)@(.+?):(\\d+?)$".toRegex()
                val match =
                    legacyPattern.matchEntire(result) ?: return ""
