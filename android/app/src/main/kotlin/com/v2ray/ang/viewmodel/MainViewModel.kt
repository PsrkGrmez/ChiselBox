...
enum class VpnStatus(val code: Int)  {
    CONNECTING(0),CONNECT(1),DISCONNECT(2)
}
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val serversPing by lazy { MutableLiveData<Pair<String, Long>>() }
    val isRunning by lazy { MutableLiveData<Boolean>() }
    val vpnStatus by lazy { MutableLiveData<VpnStatus>() }
    ...
    fun startListenBroadcast() {
        ...
        vpnStatus.value = VpnStatus.DISCONNECT
    ...
    fun testAllRealPing(configs: List<String>) {
        MessageUtil.sendMsg2TestService(getApplication(), AppConfig.MSG_MEASURE_CONFIG_CANCEL, "")
        MmkvManager.clearAllTestDelayResults()
        updateListAction.value = -1 // update all
        ...
        viewModelScope.launch(Dispatchers.Default) { // without Dispatchers.Default viewModelScope will launch in main thread
            ...
            for (config in configs) {
                val server = AngConfigManager.importBatchConfig2(config, "", true)
                if (server != null) {
                    val serverConfig = V2rayConfigUtil.getV2rayConfig2(getApplication(), server)
                    x++
                    if (serverConfig.status) {
                        MessageUtil.sendMsg2TestService(
                            getApplication(),
                            AppConfig.MSG_MEASURE_CONFIG,
                            Pair(x, serverConfig.content)
                        )
                    }
                }
            }
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                AppConfig.MSG_STATE_RUNNING -> {
                    ...
                    vpnStatus.value = VpnStatus.CONNECT
                }
