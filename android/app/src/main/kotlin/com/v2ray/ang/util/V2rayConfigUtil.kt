...
object V2rayConfigUtil {
    ...
    fun getV2rayConfig2(context: Context, server: String): Result {
        try {
            val config = MmkvManager.decodeServerConfig2(server) ?: return Result(false, "")
//            if (config.configType == EConfigType.CUSTOM) {
//                val raw = serverRawStorage?.decodeString(guid)
//                val customConfig = if (raw.isNullOrBlank()) {
//                    config.fullConfig?.toPrettyPrinting() ?: return Result(false, "")
...
//            }
            val outbound = config.getProxyOutbound() ?: return Result(false, "")
            return getV2rayNonCustomConfig(context, outbound)
        } catch (e: Exception) {
            e.printStackTrace()
            return Result(false, "")
        }
